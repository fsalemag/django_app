...
def index(request):
    ...
    n_month_activities = Activity.objects.filter(created_on__gte=datetime.now() - timedelta(days=30)).count()

    trending_categories = Category.objects.filter(
        category_activity__created_on__gte=datetime.now() - timedelta(days=7)).annotate(
